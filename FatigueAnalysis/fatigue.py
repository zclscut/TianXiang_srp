...
def fatigueFrameDetect(datatuple, frame):  # frame为摄像头原图，photo是绘制过的图片，在别的模块绘制的基础上再次绘制输出
...
if __name__ =='__main__':
    ...
    while True:
        ...
        #计算1000帧以内的闭眼时长、眨眼频率、打哈欠频率
        frametimes = 1
        for frametimes in range(1, 1001):
            # 第五步：进行循环，读取图片，并对图片做维度扩大，并进灰度化
            ret, frame = cap.read()
            frame = imutils.resize(frame, width=720)
            gray = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY)
            ...
            for rect in rects:
                ...
                cv2.putText(frame, "frame: {}".format(frametimes), (550, 150), cv2.FONT_HERSHEY_SIMPLEX, 0.7,(255, 255, 0), 2)
                '''
                    计算张嘴评分，如果小于阈值，则加1，如果连续3次都小于阈值，则表示打了一次哈欠，同一次哈欠大约在3帧
                '''
                # 同理，判断是否打哈欠
            ...
            if cv2.waitKey(1) == 27 or cv2.getWindowProperty("Frame", cv2.WND_PROP_AUTOSIZE) != 1:  # ESC的ASCII码
                break
